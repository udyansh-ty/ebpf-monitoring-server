...
// ANCHOR: Named Query Result Types - Build fix - Feb 25, 2026
// Use named types so returned slices are assignable.
...
func (q *EBPFQueries) TopDestinations(ctx context.Context, limit int) ([]DestinationStat, error) {
	sql := `
		SELECT
			dst_ip::text as ip,
			COUNT(*) as connection_count
	...
	for rows.Next() {
		...
		results = append(results, DestinationStat{IP: ip, ConnectionCount: count})
	}
...
func (q *EBPFQueries) ConnectionTimeSeries(ctx context.Context, since time.Time, until time.Time) ([]TimeSeriesStat, error) {
	sql := `
		SELECT
	...
	for rows.Next() {
		...
		results = append(results, TimeSeriesStat{Timestamp: ts, Count: count})
	}
